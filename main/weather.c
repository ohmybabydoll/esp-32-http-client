...
void http_rest_with_url(void)
{
    ...
    w_msg.name = cJSON_GetObjectItem(location, "name")->valuestring;
    w_msg.text = cJSON_GetObjectItem(now, "text")->valuestring;
    w_msg.temp = cJSON_GetObjectItem(now, "temp")->valueint;
    w_msg.rh = cJSON_GetObjectItem(now, "rh")->valueint;
    w_msg.wind_class = cJSON_GetObjectItem(now, "wind_class")->valuestring;
    w_msg.wind_dir = cJSON_GetObjectItem(now, "wind_dir")->valuestring;

    xSemaphoreGive(data_ready);

    ESP_LOGI(TAG, "城市 %s", w_msg.name);
    ESP_LOGI(TAG, "天气 %s", w_msg.text);
    ESP_LOGI(TAG, "温度 %d", w_msg.temp);
    ESP_LOGI(TAG, "湿度 %d", w_msg.rh);
    ESP_LOGI(TAG, "风力 %s", w_msg.wind_class);
    ESP_LOGI(TAG, "风向 %s", w_msg.wind_dir);

    while (xSemaphoreTake(json_done, pdMS_TO_TICKS(10) != pdTRUE))
    {
        vTaskDelay(pdMS_TO_TICKS(100)); // 等待释放资源
    }
